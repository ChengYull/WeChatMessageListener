...
- `gradle/wrapper/gradle-wrapper.properties` 中 `distributionUrl` 使用腾讯云镜像加速下载：`https://mirrors.cloud.tencent.com/gradle/gradle-9.4.1-bin.zip`。
...
- `StatsChartView.kt`——自定义 Canvas 曲线图 View（零外部依赖）。5 分钟发言频次折线图，含网格/Y 轴/X 轴标签、半透明填充、触摸指示线与气泡。通过 `setData(points, dayStart, dayEnd)` 驱动，`onDraw` 中直接 `Canvas` 绘制。
- `CalendarHeatmapView.kt`——自定义 Canvas 日历热力图 View（零外部依赖）。按月显示每日发言频次，7 列网格（日~六），红色=比前一天多，绿色=比前一天少，格子显示日期与 `+N/-N` 差值。支持左右点击切换月份，今日有橙色边框标记。
- `DateAdapter.kt`——水平日期 Chip 条适配器。展示最近 14 天 + "全部"选项，选中项高亮，切换时回调通知 Activity 更新数据源。含 `replaceDates()` 方法支持日期列表热替换（用于跨 0 点刷新）。
...
- `data/ChartPoint.kt`——分桶查询结果 POJO（`bucketStartMillis` + `count`），用于发言频次曲线图（5 分钟桶）和日历热力图（天桶）。
- `data/MessageDao.kt`——`insert`（IGNORE 冲突策略）、全量/按天 `groupsFlow()`（群聚合）、`membersFlow()`（成员聚合）、`messagesFlow()`（消息明细）、`chartFlow()`（5 分钟分桶曲线）、`dailyCountsFlow()`（按天分桶日历）、`clear()`。按天方法通过 `WHERE timestamp >= :dayStart AND timestamp < :dayEnd` 过滤。曲线查询用 `(timestamp / 300000) * 300000` 做 5 分钟桶聚合，日历查询用 `(timestamp / 86400000) * 86400000` 做天桶聚合。
...
- `MainActivity.kt`——群列表。`lifecycleScope` 收集 `groupsFlow`；`FLAG_KEEP_SCREEN_ON`；顶部日期 Chip 条用于切换按天/全量统计；选中日期时显示全部群合计的发言频次曲线图；"全部"模式显示日历热力图（按月，可切换）；按钮跳转 `Settings.ACTION_NOTIFICATION_LISTENER_SETTINGS`；`onResume` 时根据授权状态切换按钮文案并检测跨 0 点刷新日期 Chip。支持 JSON 导入（群/消息类型）与导出。
- `MemberListActivity.kt`——成员排行。接收 `EXTRA_GROUP_NAME` + 可选 `EXTRA_DAY_START/END`，按日期过滤成员发言排行。顶部显示该群聊的发言频次曲线图。支持 JSON 导入（成员/消息类型）与导出。
- `MessageListActivity.kt`——消息明细。接收 `EXTRA_GROUP_NAME` + `EXTRA_SENDER` + 可选日期范围，按日期过滤消息。顶部显示该用户的发言频次曲线图。支持 JSON 导入（消息类型）与导出。
- `data/ImportUtils.kt`——JSON 导入解析工具。支持三种类型（group/member/messages），校验 type/groupName/sender 匹配当前界面，自动兼容旧版本无 type 字段的导出文件。
- `data/ExportUtils.kt`——JSON 导出工具。导出文件含 type 标识来源界面，支持群聊和成员消息导出。
